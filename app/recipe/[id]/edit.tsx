...
export default function EditRecipeScreen() {
  ...
  const fetchRecipeDetails = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/api/recipes/${id}`
      );
      ...
      if (user?.id !== data.owner_id) {
        Alert.alert("Akses Ditolak", "Anda tidak diizinkan mengedit resep ini.");
        router.back();
      }
    } catch (e) {
      const message = e instanceof Error ? e.message : "Error tidak diketahui";
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      instructions,
      ingredients,
      image_url: image, // Keep existing image if no new one is uploaded
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Gagal memperbarui resep.");
      }
        ...
        <View className="bg-white mx-4 rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6">
          <View className="p-6 space-y-8">
            ...
            <View className="space-y-4 pt-6 border-t border-gray-200">
              <Pressable
                className={`h-14 bg-orange-500 rounded-xl shadow-lg items-center justify-center ${isSubmitting ? "opacity-70" : ""}`}
